        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredItems.map((item, index) => {
              ...
              const gridClass = isLarge ? 'md:col-span-2 lg:row-span-2' : '';
              
              return <ScrollAnimation key={item.id} animation="fade-up" delay={index * 100} className={gridClass}>
                <Card className="group relative border-0 overflow-hidden h-full bg-gradient-to-br from-white/95 to-white/90 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-700 hover:scale-[1.02]">
                  {/* Enhanced Image Container */}
                  <div className={`relative overflow-hidden ${isLarge ? 'h-96 lg:h-full' : 'h-80'}`}>
                    <img 
                      src={item.image} 
                      alt={item.title} 
                    ...
                    <div className="absolute inset-0 bg-primary/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="transform scale-0 group-hover:scale-100 transition-transform duration-500">
                        <Button 
                          ...
                          className="bg-white/90 text-primary hover:bg-white hover:scale-110 transition-all duration-300 shadow-2xl backdrop-blur-sm border-0 rounded-full px-8 py-3"
                        >
                    ...
                    {/* Corner Decorations */}
                    <div className="absolute top-0 left-0 w-16 h-16 border-l-4 border-t-4 border-white/60 rounded-tl-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    <div className="absolute top-0 right-0 w-16 h-16 border-r-4 border-t-4 border-white/60 rounded-tr-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    <div className="absolute bottom-0 left-0 w-16 h-16 border-l-4 border-b-4 border-white/60 rounded-bl-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    <div className="absolute bottom-0 right-0 w-16 h-16 border-r-4 border-b-4 border-white/60 rounded-br-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

                    {/* Media Type Badge */}
                    <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="w-14 h-14 bg-gradient-to-br from-primary to-accent rounded-full flex items-center justify-center shadow-2xl backdrop-blur-sm border border-white/20">
                    ...
                    {item.featured && 
                      <div className="absolute top-6 left-6">
                        <div className="bg-gradient-to-r from-accent to-secondary text-white px-4 py-2 rounded-full shadow-xl text-sm font-semibold flex items-center">
                  ...
                  <CardContent className="p-8 relative">
                    ...
                    <div className="absolute -top-4 left-8">
                      <Badge className="bg-gradient-to-r from-primary to-accent text-white border-0 shadow-lg px-4 py-2 font-medium">
                        {filters.find(f => f.id === item.category)?.label}
                      </Badge>
                    </div>
                    
                    <div className="pt-4">
                      <h3 className="text-2xl font-playfair font-bold text-primary mb-3 group-hover:text-accent transition-colors duration-300 leading-tight">
                        {item.title}
                      </h3>
